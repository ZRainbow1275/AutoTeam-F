...
| 版本 | v1.5 (2026-04-30 Round 11 spike — codex backend payload schema v2 落地) |
...
**关于 24h 去重 + accounts.json 字段(Round 7 已实施)**:

| 字段 | 类型 | 默认 | 含义 | 状态 |
|---|---|---|---|---|
| `last_codex_smoke_at` | float \| null | null | 上次 cheap_codex_smoke 调用的 epoch seconds | ✅ **Round 7 实施**(accounts.json 顶层落盘) |
| `last_smoke_result` | str \| null | null | `"alive"` / `"auth_invalid"` / `"uncertain"` | ✅ Round 7 实施 |
...
| `quota_info.smoke_cache_hit` | bool | false | 本次 quota_info 是 24h cache 命中产出(Round 7 新增) | ✅ Round 7 实施 |

**24h 去重契约(Round 7 v1.4 落地)**:

`check_codex_quota` 在收到 wham 200 + uninitialized_seat 形态后,在调用 `cheap_codex_smoke` 之前先按以下顺序判定:

1. 调 `_read_codex_smoke_cache(account_id)` 读 accounts.json 中该 account 对应记录的 `last_codex_smoke_at` + `last_smoke_result`
2. 若 cache 存在且 `time.time() - last_codex_smoke_at < _CODEX_SMOKE_DEDUP_SECONDS (86400)`,**直接转为 5 分类返回**(不调网络):
   - `"alive"` → `("ok", quota_info[smoke_verified=True, smoke_cache_hit=True])`
   - `"auth_invalid"` → `("auth_error", None)`
   - `"uncertain"` → `("network_error", None)`
3. 否则调 `cheap_codex_smoke(access_token, account_id)`,拿到结果后调 `_write_codex_smoke_cache(account_id, smoke_result)` 落盘 `last_codex_smoke_at = time.time()` + `last_smoke_result = smoke_result`,再按 §4.4 转 5 分类返回

**工具函数实施(`codex_auth.py` 末尾)**:
...
def _read_codex_smoke_cache(account_id):
    ...
    匹配优先级:workspace_account_id == account_id 或 email == account_id。
...
**Round 7 调用方接入点**(`check_codex_quota` 内部,9+2 调用方依旧透明):

```python
# codex_auth.py:check_codex_quota uninitialized_seat 分支(Round 7 v1.4 改造)
if status == "ok" and quota_info.get("window") == "uninitialized_seat":
    # ★ Round 7:先查 24h 去重 cache
    ...
    # cache miss / cache 过期,调网络
    smoke_result, smoke_reason = cheap_codex_smoke(access_token, account_id)
    _write_codex_smoke_cache(account_id, smoke_result)
    # ... 原 v1.3 alive/auth_invalid/uncertain 转 5 分类逻辑保留
```
...
- **I9**(Round 7 新增):**cheap_codex_smoke 在 24h cache 命中时不调用网络**。`check_codex_quota` 在 uninitialized_seat 分支必须先查 `_read_codex_smoke_cache(account_id)`,若 `time.time() - last_codex_smoke_at < 86400` 则直接转为 5 分类返回,不调 `cheap_codex_smoke` 网络。理由:fresh seat 命中率超预期(R2 风险)时若不去重,smoke 调用密度爆 → OpenAI 风控暴露。Cache miss 时调用后必须立刻写盘 `last_codex_smoke_at = time.time()` + `last_smoke_result`,保证下一次同 account 调用 24h 内命中。
